...
module.exports = {
    ...
    updateSelfies: async (req, res) => {
        ...
        if(images === undefined){
            return res.status(CODE.BAD_REQUEST).send(util.fail(CODE.BAD_REQUEST, MSG.NO_IMAGES));
        } 
        
        //image배열의 type 검사
        images.forEach(types => {
            var type = types.mimetype.split('/')[1];
            if (type !== 'jpeg' && type !== 'jpg' && type !== 'png') {
                return res.status(CODE.BAD_REQUEST).send(util.fail(CODE.BAD_REQUEST, MSG.UNSUPPORTED_TYPE));
            }
        })

        const result = await UserModel.updateSelfies(userIdx, images);

        res.status(CODE.OK).send(util.success(CODE.OK, images.length+"개 "+MSG.IMAGE_UPLOAD_SUCCESS, result));
    }
}